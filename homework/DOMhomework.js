...
nombre.innerHTML=nombre.innerHTML+'Gianella'
...
function buildToDo(todo, index) {
  ...
  var botonCheck=document.createElement('img')
  botonCheck.src="./assets/btnCheck.png"
  botonCheck.className='botonesCheck'
  botonCheck.setAttribute('id',index)

  var botonEliminar=document.createElement('img')
  botonEliminar.src="./assets/btnEliminar.png"
  botonEliminar.className='botonesCheck'

  if(todo.complete===true){
    toDoText.className='completeText'
  }
  toDoShell.appendChild(toDoText)
  toDoShell.appendChild(botonCheck)
  toDoShell.appendChild(botonEliminar)

  botonCheck.addEventListener('click' , completeToDo)
...
// Recibirá como parámetro un array de objetos ToDo que se llama toDos
...
function displayToDos() {
  ...
  var resultados =buildToDos(toDoItems)

  for (let i = 0; i < resultados.length; i++) {
    toDoContainer.appendChild(resultados[i])
